                             ...
                             uint32_t height)
{
    ...
    case DcfType::t_LPF: { // LPF 模式的曲线（低通滤波器）
        _drawLPFCurve(pos, cutoff, resonance, type, slope, width, height);
        break;
    }
    case DcfType::t_HPF: { // HPF 模式的曲线（高通滤波器）
        _drawHPFCurve(pos, cutoff, resonance, type, slope, width, height);
        break;
    }
    case DcfType::t_BPF: { // BPF 模式的曲线（带通滤波器）
        _drawBPFCurve(pos, cutoff, resonance, type, slope, width, height);
        break;
    }
    case DcfType::t_BRF: { // BRF 模式的曲线（带阻滤波器）
        _drawBRFCurve(pos, cutoff, resonance, type, slope, width, height);
        break;
    }
...
void VeeOneWidgets::_drawLPFCurve(ImVec2 &pos, float *cutoff, float *resonance, int *type, int *slope, uint32_t width,
                                  uint32_t height)
{
    //
    ...
    // 先画直线部分
    const int half_height_y = pos.y + (height >> 1); // 整数右移1位，就是除以2
    const int width_1div4 = width >> 2;              // 整数右移2位，就是除以4。后续的以此类推
    const int width_1div8 = width >> 3;
    const int width_1div16 = width >> 4;
    const int line_length = (width - width_1div4) * (*cutoff); // 直线部分的长度
    ...
    const float resonance_delta_y =
        (max_curve_height_y - min_curve_height_y) * (*resonance); // 根据当前 resonance 值，曲线最高点需要升高的纵轴高度
    ...
    int p3_x_offset = p2_x_offset >> 1; // 取 p2_offset 的一半

    // 横坐标以 line_end.x 为基准，向右偏移的量均为控件宽度的2^n分之一，旨在让曲线形状尽可能接近原版
    ImVec2 p1(line_end); // 曲线起点
    ImVec2 p2(line_end.x + (width_1div4 >> 2) + p2_x_offset,
              half_height_y); // 曲线爬升期间的拐点，旨在使曲线转折不突兀
    ImVec2 p3(line_end.x + (width_1div4 >> 1) - p3_x_offset, min_curve_height_y + resonance_delta_y); // 曲线最高点
...
void VeeOneWidgets::_drawHPFCurve(ImVec2 &pos, float *cutoff, float *resonance, int *type, int *slope, uint32_t width,
                                  uint32_t height)
{
    //
    // 高通滤波器的曲线与低通滤波器大同小异，区别在于曲线在前、直线在后。
    // 你可以把二者的曲线看成互为水平镜像。
    //
    ...
    // 先画直线部分
    const int half_height_y = pos.y + (height >> 1);
    const int width_1div4 = width >> 2;
    const int width_1div8 = width >> 3;
    const int width_1div16 = width >> 4;
    const int line_length = ((float)width - width_1div4) * (*cutoff);
...
void VeeOneWidgets::_drawBPFCurve(ImVec2 &pos, float *cutoff, float *resonance, int *type, int *slope, uint32_t width,
                                  uint32_t height)
{
    //
    // 带通滤波器的曲线画法最简单，就是一个简单的二次函数抛物线。可以直接使用单一控制点的贝塞尔曲线绘制。
    //
    ...
    const float resonance_delta_y =
        (max_curve_height_y - min_curve_height_y) * (*resonance); // 根据当前 resonance 值，曲线中间点需要升高的纵轴高度
    ...
    const float curve_left_begin_x =
        ...
        (max_curve_x - min_curve_x) * (*cutoff); // 根据当前 cutoff 值，计算出曲线左侧控制点的坐标
...
void VeeOneWidgets::_drawBRFCurve(ImVec2 &pos, float *cutoff, float *resonance, int *type, int *slope, uint32_t width,
                                  uint32_t height)
{
    //
    ...
    // 通用的量，画左右半边曲线都要用到。
    const float left_line_length = float(width - width_1div4 - width_1div8) * (*cutoff); // 左侧直线的长度
    const float min_curve_height_y = (float)half_height_y;         // 这个y值，对应的是控件的一半高度
    const float max_curve_height_y = (float)(pos.y - height + 20); // 这个y值，对应的是控件顶部，略往下一点点
    ...
    const float curve_base_delta = (float)(width - 2 * (width_1div8 + float(width_1div16 >> 1))) * ((*cutoff - 0.5));
    ...
    //
    {
        ...
        const float resonance_delta_y = (max_curve_height_y - min_curve_height_y) * (*resonance);
        ...
        ImVec2 p1(((FilterSlope(*slope) == FilterSlope::s_24dBPerOct) ? curve_begin.x - width_1div16 : curve_begin.x),
                  curve_begin.y);
                                      ...
                                      uint32_t width, uint32_t height)
{
...

    {
        ...
        int x = width_1div8 + int((*cutoff) * float(width - width_1div4));

        // y 坐标代表了共振频率在控件高度上的位置，它决定了formant曲线的垂直位置
        int y = half_height - int((*resonance) * float(height + height_1div4));
        ...
        //
        // 共振峰曲线由五段首尾相接的曲线构成。
        // 接下来通过循环，依次绘制这五段曲线。下面的逻辑直接从原版移植过来。
        //
        ImVec2 last_p0(pos.x, pos.y + half_height); // 上一段曲线的起点
