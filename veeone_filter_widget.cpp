...
void DCFCurve(const char *id, float *cutoff, float *resonance, int *type, int *slope, uint32_t width, uint32_t height)
{
    ...
    case DcfType::t_BRF: { // BRF 模式的曲线（带阻滤波器）
        ...
        // 基础的常量。
        const float half_height_y = pos.y + (float)height / 2;
        const float half_width_x = pos.x + (float)width / 2;
        const float width_1div4 = (float)width / 4;
        const float width_1div8 = (float)width / 8;
        const float width_1div16 = (float)width / 16;
        ...
        const float curve_base_delta = (float)(width - 2 * (width_1div8 + width_1div16 / 2)) * ((*cutoff - 50) * 0.01);
        ...
        //
        {
            ...
            ImVec2 p1(
                ...
                curve_begin.y);
            ImVec2 p2(curve_begin.x - width_1div8 / 2 - p2_x_offset,
                      min_curve_height_y + resonance_delta_y); // 曲线最高点
            ImVec2 p3(p2.x - width_1div16 / 4 - p3_x_offset,
                      half_height_y); // 曲线爬升期间的拐点，旨在使曲线转折不突兀
        ...
        // 考虑到这一特殊性，我们需要先画曲线，剩下空间足够的话再画直线。
        {
            ...
            ImVec2 p1(
                ((FilterSlope(*slope) == FilterSlope::s_24dBPerOct) ? curve_begin.x + width_1div16 : curve_begin.x),
                curve_begin.y);
            ImVec2 p2(curve_begin.x + width_1div8 / 2 + p2_x_offset, min_curve_height_y); // 曲线最高点
            ImVec2 p3(p2.x + width_1div16 / 4 + p3_x_offset, half_height_y); // 曲线爬升期间的拐点，旨在使曲线转折不突兀
            ImVec2 p4(p3.x + width_1div16, half_height_y);
