...
int main(int, char**)
{
    ...
    std::unique_ptr<padthv1_sample> test_sampledata = std::make_unique<padthv1_sample>(nullptr);
    test_sampledata->reset(0, 40, 0, 32, padthv1_sample::Rect);
    ...
    while (!glfwWindowShouldClose(window))
    {
...

        {
            ...
            static float sustain = 0.5f;
            static float release = 1.0f;
...

        {
            ...
            static float wave_width = 1.0f;

            {
                VeeOneWidgets::OscWave("osc1", &*test_wavedata, &waveform, &wave_width, 100, 100);
            }
            ...
            ImGui::BeginGroup();
            {
                ...
                ImGui::RadioButton("Pulse", &waveform, synthv1_wave_lf::Pulse); ImGui::SameLine();
                ImGui::RadioButton("Saw", &waveform, synthv1_wave_lf::Saw); ImGui::SameLine();
                ImGui::RadioButton("Sine", &waveform, synthv1_wave_lf::Sine); ImGui::SameLine();
                ImGui::RadioButton("Random", &waveform, synthv1_wave_lf::Rand); ImGui::SameLine();
                ImGui::RadioButton("Noise", &waveform, synthv1_wave_lf::Noise);
            }
            ImGui::EndGroup();

            ImGui::SliderFloat("Width##Waveform", &wave_width, 0.0f, 1.0f);

            ImGui::End();
        }

        {
            ImGui::Begin("Sample wave form test");

            static int freq0 = 0;
            static float sample_width = 40;
            static float scale = 0;
            static uint16_t nh = 32;
            static int arpodizer = padthv1_sample::Rect;

            {
                VeeOneWidgets::SampleWave("test_sample_waveform", &*test_sampledata, &freq0, &sample_width, &scale, &nh, 400, 160);
            }

            bool s1 = ImGui::SliderInt("Freq", &freq0, 0, 127);
            bool s2 = ImGui::SliderFloat("Width", &sample_width, 2.0f, 200.0f);
            bool s3 = ImGui::SliderFloat("Scale", &scale, -1.0f, 1.0f);

            const uint16_t nh_min = 2, nh_max = 64;
            bool s4 = ImGui::SliderScalar("NH", ImGuiDataType_U16, &nh, &nh_min, &nh_max);
            if (s1 || s2 || s3 || s4) {
                test_sampledata->reset(freq0, sample_width, scale, nh, padthv1_sample::Apodizer(arpodizer));
            }

            ImGui::BeginGroup();
            {
                ImGui::Text("Arpodizer:");

                bool s1 = ImGui::RadioButton("Rect", &arpodizer, padthv1_sample::Rect); ImGui::SameLine();
                bool s2 = ImGui::RadioButton("Triangle", &arpodizer, padthv1_sample::Triang); ImGui::SameLine();
                bool s3 = ImGui::RadioButton("Welch", &arpodizer, padthv1_sample::Welch); ImGui::SameLine();
                bool s4 = ImGui::RadioButton("Hann", &arpodizer, padthv1_sample::Hann); ImGui::SameLine();
                bool s5 = ImGui::RadioButton("Gauss", &arpodizer, padthv1_sample::Gauss);
